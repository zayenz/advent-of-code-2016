...
fn distance(instructions: Vec<(Turn, i32)>) -> Result<i32, Error> {
    ...
    for (turn, steps) in instructions {
        ...
        for _ in 0..steps {
            position = position.walk(direction, 1);
            if visited.contains(&position) {
                return Ok(position.distance_to_origo());
            }
            visited.insert(position);
        }
    }
...
fn run() -> Result<(), Error> {
    ...
    let distance = distance(instructions)?;
